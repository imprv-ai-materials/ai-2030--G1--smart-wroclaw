...
So a test must (1) persist the user and (2) sign a matching token — this
...
def auth_headers(user_id: int, email: str, config: "Config") -> dict[str, str]:
    """Return an `Authorization` header for a user (`X-User-Id` free)."""
    token, _ = issue_access_token(user_id, email, config)
