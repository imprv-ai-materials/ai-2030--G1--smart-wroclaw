...
class AbstractAuthTokensRepository(abc.ABC):
    @abc.abstractmethod
    def create(
        self, citizen_id: int, kind: TokenKind, token_hash: str, expires_at: datetime
    ) -> AuthToken: ...
    ...
    @abc.abstractmethod
    def invalidate_all(self, citizen_id: int, kind: TokenKind) -> None:
...
class AuthTokensRepository(AbstractAuthTokensRepository):
    ...
    def invalidate_all(self, citizen_id: int, kind: TokenKind) -> None:
        self._db.update_one(
            auth_tokens_table,
            {"citizen_id": citizen_id, "kind": kind.value, "used_at__isnull": True},
            {"used_at": datetime.now(tz=timezone.utc)},
        )
