"""Public citizen event feed + the authenticated "file an event" write.
    ...
    POST /events/ingest          bulk seed (specialist key) — demo/data loading
"""
...
from api.contexts_boundaries.city_events_bc.rest.events_schemas import (
    EventCreateRequest,
    EventIngestRequest,
    EventListResponse,
)
...
def prolong_event(
    event_id: int,
    current: Citizen = Depends(require_confirmed_citizen),
    bootstrap: Bootstrap = Depends(get_bootstrap_dep),
) -> CityEvent:
...
@events_router.post("/ingest", response_model=EventListResponse, status_code=status.HTTP_201_CREATED)
def ingest_events(
    body: EventIngestRequest,
    _spec: SpecialistContext = Depends(specialist),
    bootstrap: Bootstrap = Depends(get_bootstrap_dep),
) -> EventListResponse:
    created = bootstrap.events_service.ingest([e.model_dump() for e in body.events])
    return EventListResponse(events=created)
