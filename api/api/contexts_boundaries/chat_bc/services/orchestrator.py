...
`_route`, `_search`, `_add`, `build_reply`). The synchronous driver `run_turn`
composes them for the interactive chat REST endpoint (see `router.py`); the
durable Inngest variant that wraps each element in its own `ctx.step.run` lives
in `chat_bc/inngest_functions.py`.
...
def _create(bootstrap: Bootstrap, draft: dict[str, Any], reporter_id: int) -> CityEvent:
    """Persist a completed draft as a user report. Coordinates are enriched
