"""Generate the event-categorisation dataset for the DeepEval loop.

Run:  python api/api/ai/event_extractor/eval/generate_dataset.py   (from the repo root)

It emits, into api/api/ai/event_extractor/datasets/:
...
It is a STARTING POINT — enhance it (e.g. in the /.annotator UI), then re-run.
Split is deterministic (no RNG) so re-generating is stable.
"""
