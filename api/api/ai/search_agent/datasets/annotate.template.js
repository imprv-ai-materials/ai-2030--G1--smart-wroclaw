/* Annotator template for the SEARCH_AGENT dataset (search.jsonl) — 4-column case.

   Context — this agent's self-contained mini-corpus (corpus.jsonl); each card has
             quick "+bucket" buttons. (The authoritative 38-event retrieval gold
             lives at api/api/ai/main_agent/datasets — this seed is for isolating
             the ranker.)
   Input   — the resident query + the gold structured reading.
   Output  — ordered buckets: relevant / acceptable / hard_negatives / must_exclude.

   Run:  python .annotator/annotate.py api/api/ai/search_agent/datasets/annotate.template.js api/api/ai/search_agent/datasets/search.jsonl */
...
export const meta = {
  title: "Annotating the Search Agent Dataset",
...
export const panes = {
  ...
  input(host, ctx) {
    ...
    host.append(
      ...
      w.text(row.query, (v) => ctx.persist((r) => (r.query = v)), { rows: 8, placeholder: "np. zaginął mi kot na Krzykach" }),
      h("p", { class: "pane-hint" }, "The only input. The right side is what the search agent should rank/return."),
    );
  },
  ...
  output(host, ctx) {
    ...
    host.append(
      section(ctx, 1, "Expected extraction", "The structured reading the search agent gets as input."),
