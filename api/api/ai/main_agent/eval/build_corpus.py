...
Regenerate any time the demo feed changes:
    ...
    python api/api/ai/main_agent/eval/build_corpus.py

Then re-check the query labels in `datasets/queries.jsonl` still point at the
right `ext_id`s (they are index-stable as long as rows aren't reordered).
"""
...
OUT = Path(__file__).resolve().parents[1] / "datasets" / "corpus.jsonl"
