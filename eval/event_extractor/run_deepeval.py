"""Run the DeepEval loop for category_agent and print scores.

    python eval/category_agent/run_deepeval.py                 # dev split, offline
    python eval/category_agent/run_deepeval.py --split test     # held-out (touch rarely!)
    python eval/category_agent/run_deepeval.py --split all      # dev + test + the gap
    python eval/category_agent/run_deepeval.py --llm            # use the OpenAI path
...
ROOT = Path(__file__).resolve().parents[2]  # smart_wroclaw/
...
def _load(split: str) -> list[dict]:
    ...
    if not path.is_file():
        sys.exit(f"missing {path} — run: python eval/event_extractor/generate_dataset.py")
    return [json.loads(line) for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]


def _build_agent(use_llm: bool):
    current = load_current("event_extractor")
    api_key = config.openai.api_key if use_llm else None
    client = OpenAIClient(api_key=api_key, default_model_name=config.openai.default_model_name)
    model = current.model or config.openai.default_model_name
    agent = current.agent_class(client, model=model)
    mode = "llm" if (use_llm and client.is_configured) else "offline-keyword"
    return agent, current.version, mode
